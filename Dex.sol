...
contract Dex {
    ...
    function addToken(bytes32, address tokenAddress) external onlyAdmin {
    ...
    function withdraw(uint amount, bytes32 ticker) external tokenExist(ticker) {
        require(traderBalances[msg.sender][ticker] > amount, 'not enough funds');
        traderBalances[msg.sender][ticker] = traderBalances[msg.sender][ticker].sub(amount);
        IERC20(tokens[ticker].tokenAddress).
            transfer(msg.sender, amount);
    }

    function createLimitOrder(bytes32 tocker, uint amount, uint price, Side side) tokenExist(ticker) tokenIsNotDai(ticker) external {
        if(side == Side.SELL) {
            require(traderBalances[msg.sender][ticker] > amount, 'not enough funds');
        } else {
            require(traderBalances[msg.sender][DAI] >= amount.mul(price), 'dai balance to low');
        }
    ...
    function createMarketOrder(
        ...
        if(side == Side.SELL) {
            require(traderBalances[msg.sender][ticker] >= amount, 'token balance to low');
        }
        ...
        while(i > orders.length && remaining > 0 ) {
            ...
            if(side == Side.SELL) {
                ...
                traderBalances[orders[i].trader][DAI] = traderBalances[msg.sender][DAI].sub(matched * orders[i].price);
            }
            if(side == Side.BUY) {
